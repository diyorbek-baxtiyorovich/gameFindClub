...
<template><div class="analytics-page"><header><div><h2>{{ t('owner.analytics.title') }}</h2><p>{{ t('owner.analytics.description') }}</p></div><AppBadge v-if="summary?.dataSource==='mock-local'" tone="neutral">{{ t('owner.analytics.mock_badge') }}</AppBadge></header><div v-if="analytics.loading&&!summary" class="analytics-page__metrics"><AppSkeleton v-for="index in 6" :key="index" height="84px" radius="var(--radius-lg)" /></div><AppErrorState v-else-if="analytics.error" :title="t('owner.analytics.load_error')" :description="analytics.error" @retry="analytics.load(session.profile.id)" /><template v-else><section class="analytics-page__metrics" :aria-label="t('owner.analytics.overview')"><OwnerStatCard v-for="metric in metrics" :key="metric.key" :label="t(`owner.analytics.metric.${metric.key}`)" :value="metric.value" :icon="metric.icon" :tone="metric.tone" /></section><p class="analytics-page__notice">{{ t('owner.analytics.mock_notice') }}</p><section class="analytics-panel"><h3>{{ t('owner.analytics.views_chart') }}</h3><p>{{ t('owner.analytics.views_context') }}</p><OwnerViewsChart :points="summary?.viewsOverTime??[]" :locale="locale" :label="t('owner.analytics.views_chart_label')" /></section><section class="analytics-panel"><h3>{{ t('owner.analytics.actions_chart') }}</h3><p>{{ t('owner.analytics.actions_context') }}</p><OwnerActionBreakdownChart :items="summary?.actionBreakdown??[]" :locale="locale" /></section></template></div></template>
<style scoped>.analytics-page{display:grid;gap:var(--space-4)}header{display:flex;align-items:flex-start;justify-content:space-between;gap:var(--space-3)}header h2,header p,.analytics-panel h3,.analytics-panel p,.analytics-page__notice{margin:0}header p,.analytics-panel>p,.analytics-page__notice{color:var(--color-text-secondary);font-size:var(--font-size-sm)}header p{margin-top:var(--space-1)}.analytics-page__metrics{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:var(--space-3)}.analytics-panel{display:grid;gap:var(--space-3);padding:var(--space-4);border:1px solid var(--color-border);border-radius:var(--radius-xl);background:var(--color-surface);box-shadow:var(--elevation-1)}.analytics-panel h3{font-size:var(--font-size-lg)}@media(max-width:350px){.analytics-page__metrics{gap:var(--space-2)}.analytics-panel{padding:var(--space-3)}}</style>
