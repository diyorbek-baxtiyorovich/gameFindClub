...
<template><section class="reviews-page"><header><h1>{{ t('reviews.title') }}</h1><p>{{ t('reviews.description') }}</p></header><CategorySelector include-all :selected-id="selectedCategoryId" @select="selectedCategoryId=$event"/><ReviewList :items="filtered" :loading="reviews.loading" :error="reviews.error" @retry="reviews.load"/><ReviewForm v-if="venue" :venue-id="venue.id" :category-code="venue.category.slug" @submit="reviews.submit"/></section></template>
