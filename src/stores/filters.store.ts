...
const isFilterArray = (value: DynamicFilterValue): value is readonly string[] | readonly number[] =>
  Array.isArray(value)
...
export const useFiltersStore = defineStore('filters', () => {
  ...
  const category = computed(() =>
    categoryId.value ? getCategoryById(categoryId.value) : UNKNOWN_CATEGORY,
  )
  ...
  const activeFilters = computed(() => {
    ...
    if (radius.value !== DEFAULT_RADIUS)
      entries.push({ key: 'radius', label: `${radius.value} km` })
    if (openNow.value) entries.push({ key: 'openNow', label: 'Open now' })
    if (minRating.value != null)
      entries.push({ key: 'minRating', label: `${minRating.value}+ rating` })
    if (hasRange(priceRange.value)) entries.push({ key: 'priceRange', label: 'Price range' })
    if (amenities.value.length)
      entries.push({ key: 'amenities', label: `${amenities.value.length} amenities` })
    ...
    for (const schema of categorySchemas.value) {
      if (hasDynamicValue(dynamic.value[schema.key]))
        entries.push({ key: schema.key, label: schema.label })
    }
    return entries
  })
  ...
  function setCategory(nextCategoryId: string | null): void {
    ...
    const validKeys = new Set(
      (nextCategoryId ? (getCategoryById(nextCategoryId).filters ?? []) : []).map(
        (schema) => schema.key,
      ),
    )
    dynamic.value = Object.fromEntries(
      Object.entries(dynamic.value).filter(([key]) => validKeys.has(key)),
    )
  }
