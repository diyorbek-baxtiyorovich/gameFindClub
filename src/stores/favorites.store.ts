...
export const useFavoritesStore = defineStore('favorites', () => {
  ...
  const items = computed<readonly VenueListItem[]>(() => ids.value.map((id) => fixtureItems.find((venue) => venue.id === id)).filter((venue): venue is VenueListItem => Boolean(venue)).map((venue) => ({ ...venue, isFavorite: true })))
