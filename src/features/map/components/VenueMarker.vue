...
function renderMarker(): void {
  ...
  const colors: Readonly<Record<string, string>> = { gaming: '#8b5cf6', cyan: '#22d3ee', neutral: '#64748b' }
  ...
  const symbolStyle = 'display:block;transform:rotate(45deg);font-size:12px;font-weight:800'
  const availability = props.venue.occupancy?.availableCount ?? 0
  const icon = L.divIcon({ className: 'venue-marker-shell', html: `<span class="${classes}" style="${markerStyle}" aria-hidden="true"><b style="${symbolStyle}">${availability}</b></span>`, iconSize: [48, 54], iconAnchor: [24, 50] })
...
<style>:global(.venue-marker-shell) { background: transparent; border: 0; }:global(.venue-marker) { --marker-accent: var(--color-accent-neutral); width: 40px; height: 40px; display: grid; place-items: center; border: 3px solid var(--color-surface); border-radius: 50% 50% 50% 12px; transform: rotate(-45deg); color: white; background: var(--marker-accent); box-shadow: var(--elevation-2); font-weight: var(--font-weight-bold); }:global(.venue-marker > span) { transform: rotate(45deg); }:global(.venue-marker--gaming) { --marker-accent: var(--color-accent-gaming); }:global(.venue-marker--cyan) { --marker-accent: var(--color-accent-cyan); }:global(.venue-marker--selected) { outline: 4px solid color-mix(in srgb, var(--color-primary) 35%, transparent); transform: rotate(-45deg) scale(1.14); }:global(.venue-marker--featured) { box-shadow: 0 0 0 3px var(--color-warning), var(--elevation-2); }:global(.venue-marker--unavailable) { filter: grayscale(0.85); opacity: 0.58; }</style>
