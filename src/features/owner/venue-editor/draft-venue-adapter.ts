...
const persistentMedia = (media: readonly VenueMedia[]): readonly VenueMedia[] =>
  media.filter((item) => !/^(?:blob:|data:)/.test(item.url))
...
function editableFields(draft: VenueEditorDraft): UpdateVenueInput {
  ...
  const coverMedia =
    draft.coverMedia && !/^(?:blob:|data:)/.test(draft.coverMedia.url) ? draft.coverMedia : media[0]
  const hasCoordinates =
    draft.coordinates.latitude !== undefined && draft.coordinates.longitude !== undefined
  const formatted =
    draft.address.formatted.trim() ||
    [draft.address.district, draft.address.city].filter(Boolean).join(', ')
  return {
    name: draft.name.trim(),
    shortDescription: draft.shortDescription.trim(),
    description: draft.description.trim(),
    category: categorySummaryForDraft(draft),
    categoryAttributes: draft.categoryAttributes,
    address: formatted
      ? {
          formatted,
          city: draft.address.city.trim() || undefined,
          district: draft.address.district.trim() || undefined,
        }
      : undefined,
...
export function draftToUpdateInput(draft: VenueEditorDraft): UpdateVenueInput {
  return editableFields(draft)
}
...
export function draftToPreviewVenue(draft: VenueEditorDraft, fallbackName: string): VenueDetail {
  ...
  return {
    id: draft.sourceVenueId ?? 'owner-draft-preview',
    slug: fields.slug ?? 'owner-draft-preview',
    name: fields.name || fallbackName,
    category: fields.category ?? categorySummaryForDraft(draft),
