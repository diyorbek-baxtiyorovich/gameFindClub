...
import { isValidInstagram, isValidPhone, isValidTelegram, isValidWebUrl } from './contact-validation'
...
export function validateBasicInformation(draft: VenueEditorDraft): Partial<Record<VenueBasicField, MessageKey>> {
  ...
  else if (nameLength < VENUE_VALIDATION_LIMITS.nameMin) errors.name = 'owner.editor.validation.name_min'
  else if (nameLength > VENUE_VALIDATION_LIMITS.nameMax) errors.name = 'owner.editor.validation.name_max'
  if (draft.shortDescription.length > VENUE_VALIDATION_LIMITS.shortDescriptionMax) errors.shortDescription = 'owner.editor.validation.short_max'
  if (draft.description.length > VENUE_VALIDATION_LIMITS.descriptionMax) errors.description = 'owner.editor.validation.description_max'
  return errors
}

export function validateContacts(contacts?: ContactInfo): Partial<Record<VenueContactField, MessageKey>> {
  const errors: Partial<Record<VenueContactField, MessageKey>> = {}
  if (contacts?.phone && !isValidPhone(contacts.phone)) errors.phone = 'owner.editor.validation.phone'
  if (contacts?.secondaryPhone && !isValidPhone(contacts.secondaryPhone)) errors.secondaryPhone = 'owner.editor.validation.phone'
  if (contacts?.telegramUsername && !isValidTelegram(contacts.telegramUsername)) errors.telegram = 'owner.editor.validation.telegram'
  const instagram = contacts?.socialLinks?.instagram
  if (instagram && !isValidInstagram(instagram)) errors.instagram = 'owner.editor.validation.instagram'
  if (contacts?.websiteUrl && !isValidWebUrl(contacts.websiteUrl)) errors.website = 'owner.editor.validation.url'
  return errors
}

export function validateLocation(draft: VenueEditorDraft): Partial<Record<VenueLocationField, MessageKey>> {
  ...
  if (latitude == null || !Number.isFinite(latitude) || latitude < -90 || latitude > 90) errors.latitude = 'owner.editor.validation.latitude'
  if (longitude == null || !Number.isFinite(longitude) || longitude < -180 || longitude > 180) errors.longitude = 'owner.editor.validation.longitude'
  return errors
}

export function validatePricingOption(option: PricingOption): Partial<Record<'name'|'amount'|'description', MessageKey>> {
  ...
  if (!Number.isFinite(option.price.amount) || option.price.amount <= 0 || option.price.currency !== 'UZS') errors.amount = 'owner.editor.validation.amount_positive'
  if (option.description && option.description.length > VENUE_VALIDATION_LIMITS.pricingDescriptionMax) errors.description = 'owner.editor.validation.price_description_max'
  return errors
}
...
export function isScheduleSubmissionReady(draft: VenueEditorDraft): boolean {
  return Boolean(draft.schedule && isOpeningHoursValid(draft.schedule)
...
export function isMediaDraftValid(draft: VenueEditorDraft): boolean {
  ...
  return draft.media.every((item) => item.type === 'image' && !item.url.startsWith('data:'))
    && (!draft.coverMedia || ids.has(draft.coverMedia.id))
}

export function isVenueEditorStepValid(stepId: VenueEditorStepId, draft: VenueEditorDraft): boolean {
  ...
  if (stepId === 'category-features') return areCategoryFieldsValid(getCategoryById(draft.categoryId).editorFields ?? [], draft.categoryAttributes)
...
export type SubmissionIssue = 'name'|'basic'|'category'|'address'|'coordinates'|'category_fields'|'schedule'|'pricing'|'contacts'|'media'
...
export function validateVenueSubmission(draft: VenueEditorDraft): readonly SubmissionIssue[] {
  ...
  if (!areCategoryFieldsValid(getCategoryById(draft.categoryId).editorFields ?? [], draft.categoryAttributes)) issues.push('category_fields')
