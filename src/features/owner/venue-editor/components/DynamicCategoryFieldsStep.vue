...
const label = (field: FilterSchema): string =>
  field.labelKey ? t(field.labelKey as MessageKey) : field.label
const optionLabel = (option: FilterOption): string =>
  option.labelKey ? t(option.labelKey as MessageKey) : option.label
const valueFor = (field: FilterSchema): VenueCategoryAttributeValue | undefined =>
  props.draft.categoryAttributes[field.key]
...
function errorFor(field: FilterSchema): string | undefined {
  ...
  if (field.required && (value == null || value === '' || (Array.isArray(value) && !value.length)))
    return t('owner.editor.validation.field_required')
  if (field.control === 'number' && value != null) {
    if (typeof value !== 'number' || !Number.isFinite(value))
      return t('owner.editor.validation.number')
    if (field.min != null && value < field.min)
      return t('owner.editor.validation.number_min', { min: field.min })
    if (field.max != null && value > field.max)
      return t('owner.editor.validation.number_max', { max: field.max })
  }
  return undefined
}
...
function toggleMulti(field: FilterSchema, option: FilterOption): void {
  ...
  if (typeof option.value === 'string') {
    const values = Array.isArray(current)
      ...
      : []
    draftStore.setCategoryAttribute(
      field.key,
  ...
  <div class="dynamic-fields-step">
    ...
    <template v-for="field in fields" :key="field.key">
      <AppInput
        ...
        @update:model-value="draftStore.setCategoryAttribute(field.key, $event)"
      />
      <AppInput
        v-else-if="field.control === 'number'"
        :model-value="valueFor(field)?.toString() ?? ''"
        type="number"
        :label="label(field)"
        :hint="field.unitLabel"
        :error="errorFor(field)"
        :required="field.required"
        :min="field.min"
        :max="field.max"
      ...
      />
      <AppCheckbox
        v-else-if="field.control === 'toggle' || field.control === 'boolean'"
      ...
      />
      <AppSelect
        v-else-if="field.control === 'single-select' || field.control === 'select'"
        :model-value="selectedValue(field)"
        :label="label(field)"
      ...
      <fieldset v-else-if="field.control === 'multi-select'" class="dynamic-fields-step__multi">
        ...
        <div>
          <AppChip
            ...
            :selected="isSelected(field, option)"
            :disabled="option.disabled"
            @click="toggleMulti(field, option)"
            >{{ optionLabel(option) }}</AppChip
          >
        </div>
    ...
    <p v-if="!fields.length" class="dynamic-fields-step__empty">
      {{ t('owner.editor.features.empty') }}
    </p>
  </div>
</template>
...
.dynamic-fields-step header h3,
.dynamic-fields-step header p {
  margin: 0;
}
.dynamic-fields-step header p,
.dynamic-fields-step__empty {
  margin-top: var(--space-2);
  color: var(--color-text-secondary);
}
.dynamic-fields-step__multi {
  display: grid;
  gap: var(--space-2);
  margin: 0;
  padding: 0;
...
.dynamic-fields-step__multi legend {
  margin-bottom: var(--space-2);
  font-size: var(--font-size-sm);
  font-weight: var(--font-weight-medium);
}
.dynamic-fields-step__multi > div {
  display: flex;
  flex-wrap: wrap;
  gap: var(--space-2);
}
.dynamic-fields-step__multi small {
