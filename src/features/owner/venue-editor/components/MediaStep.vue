...
import { t } from '@/i18n'
import { ownerMediaSelectionService, type LocalMediaSelectionError } from '@/services/owner-media-selection'
...
function selectFiles(event: Event): void {
  ...
  const media = [...props.draft.media, ...result.media].map((item, index) => ({ ...item, sortOrder: index }))
...
function remove(media: VenueMedia): void {
  ...
  const items = props.draft.media.filter((item) => item.id !== media.id).map((item, index) => ({ ...item, sortOrder: index }))
...
function move(index: number, direction: -1 | 1): void {
  ...
  draftStore.setMedia(items.map((item, itemIndex) => ({ ...item, sortOrder: itemIndex })), props.draft.coverMedia)
}
</script>
  ...
  <div class="media-step">
    ...
    <div v-if="draft.media.length" class="media-step__grid">
      <article v-for="(media, index) in draft.media" :key="media.id" class="media-step__item">
        ...
        <div>
          <button type="button" :disabled="draft.coverMedia?.id === media.id" @click="setCover(media)">{{ t('owner.editor.media.make_cover') }}</button>
          <button type="button" :aria-label="t('owner.editor.media.move_up')" :disabled="index === 0" @click="move(index, -1)">↑</button>
          <button type="button" :aria-label="t('owner.editor.media.move_down')" :disabled="index === draft.media.length - 1" @click="move(index, 1)">↓</button>
          <button type="button" class="media-step__remove" :aria-label="t('owner.editor.media.remove')" @click="remove(media)"><AppLucideIcon name="x" :size="17" /></button>
        </div>
      </article>
    </div>
...
.media-step__item { position: relative; overflow: hidden; border: 1px solid var(--color-border); border-radius: var(--radius-lg); background: var(--color-surface); }
.media-step__item img { width: 100%; aspect-ratio: 4 / 3; display: block; object-fit: cover; }
.media-step__item > span { position: absolute; top: var(--space-2); left: var(--space-2); padding: 2px var(--space-2); border-radius: var(--radius-pill); color: var(--color-text-inverse); background: var(--color-primary); font-size: var(--font-size-xs); font-weight: var(--font-weight-semibold); }
.media-step__item > div { display: flex; align-items: center; padding: var(--space-1); }
.media-step__item button { min-width: 38px; min-height: var(--touch-target-min); border: 0; color: var(--color-primary); background: transparent; cursor: pointer; font-size: var(--font-size-xs); }
.media-step__item button:first-child { flex: 1; }
.media-step__item button:disabled { opacity: .45; cursor: default; }
.media-step__item .media-step__remove { color: var(--color-danger); }
@media (max-width: 350px) { .media-step__grid { grid-template-columns: 1fr; } }
</style>
