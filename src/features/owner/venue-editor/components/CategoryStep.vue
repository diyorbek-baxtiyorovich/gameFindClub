...
<template>
  <div class="category-step">
    ...
    <CategorySelector
      :selected-id="pendingCategoryId ?? draft.categoryId"
      @select="selectCategory"
    />
    <p v-if="!draft.categoryId" class="category-step__error" role="alert">
      {{ t('owner.editor.validation.category_required') }}
    </p>
    ...
    <section v-if="pendingCategoryId" class="category-step__confirmation" role="alert">
      ...
      <div class="category-step__actions">
        <AppButton variant="secondary" @click="cancelCategoryChange">{{
          t('owner.action.cancel')
        }}</AppButton>
        <AppButton @click="confirmCategoryChange">{{
          t('owner.editor.category.change_confirm')
        }}</AppButton>
      </div>
    </section>
  </div>
...
.category-step > div > p,
.category-step__confirmation p {
  margin-top: var(--space-2);
  color: var(--color-text-secondary);
}
.category-step__error {
  color: var(--color-danger);
  font-size: var(--font-size-sm);
}
.category-step__confirmation {
  display: grid;
  gap: var(--space-3);
  padding: var(--space-4);
