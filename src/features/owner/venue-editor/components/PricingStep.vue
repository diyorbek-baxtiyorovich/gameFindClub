...
function updateOption(id: string, update: PricingUpdater): void {
  draftStore.setPricing(
    props.draft.pricing.map((option) => (option.id === id ? update(option) : option)),
  )
}
...
<template>
  <div class="pricing-step">
    <header>
      <div>
        <h3>{{ t('owner.editor.pricing.title') }}</h3>
        <p>{{ t('owner.editor.pricing.description') }}</p>
      </div>
      <AppButton size="sm" variant="secondary" @click="addOption"
        ><template #leading><AppLucideIcon name="plus" /></template
        >{{ t('owner.editor.pricing.add') }}</AppButton
      >
    </header>

    <div v-if="draft.pricing.length" class="pricing-step__list">
      <article v-for="(option, index) in draft.pricing" :key="option.id">
        <div class="pricing-step__card-header">
          <strong>{{ t('owner.editor.pricing.option', { number: index + 1 }) }}</strong
          ><button
            type="button"
            :aria-label="t('owner.editor.pricing.remove')"
        ...
        <AppInput
          ...
          :label="t('owner.editor.pricing.name')"
          :placeholder="t('owner.editor.pricing.name_placeholder')"
          :error="optionError(option, 'name')"
          required
          @update:model-value="updateOption(option.id, (current) => ({ ...current, name: $event }))"
        />
        <div class="pricing-step__row">
          <AppInput
            :model-value="option.price.amount > 0 ? option.price.amount.toString() : ''"
            type="number"
            :label="t('owner.editor.pricing.amount')"
            :min="1"
            :step="1000"
            :error="optionError(option, 'amount')"
            required
            @update:model-value="updateAmount(option, $event)"
          />
          <div class="pricing-step__currency">
            <span>{{ t('owner.editor.pricing.currency') }}</span
            ><strong>UZS</strong>
          </div>
        </div>
        <AppSelect
          :model-value="option.unit"
          :label="t('owner.editor.pricing.unit')"
          :options="unitOptions"
          @update:model-value="updateUnit(option, $event)"
        />
        <AppTextarea
          :model-value="option.description ?? ''"
          :label="t('owner.editor.pricing.option_description')"
          :rows="3"
          :maxlength="300"
          :error="optionError(option, 'description')"
          @update:model-value="
            updateOption(option.id, (current) => ({ ...current, description: $event || undefined }))
          "
        />
      </article>
    </div>
...
.pricing-step__card-header {
  display: flex;
  align-items: center;
  justify-content: space-between;
}
.pricing-step__card-header button {
...
.pricing-step__currency {
  display: grid;
  align-content: start;
  gap: var(--space-2);
}
.pricing-step__currency span {
  font-size: var(--font-size-sm);
  font-weight: var(--font-weight-medium);
}
.pricing-step__currency strong {
