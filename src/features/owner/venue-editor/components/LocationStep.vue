...
const locationError = computed(() => {
  ...
  if (geolocation.status === 'unavailable' || geolocation.status === 'error')
    return t('owner.editor.location.unavailable')
  return null
})
...
function updateCoordinate(key: 'latitude' | 'longitude', value: string): void {
  ...
  const withinRange =
    key === 'latitude' ? parsed >= -90 && parsed <= 90 : parsed >= -180 && parsed <= 180
...
<template>
  <div class="location-step">
    <AppButton
      variant="secondary"
      :loading="geolocation.status === 'requesting'"
      @click="useCurrentLocation"
    >
      <template #leading><AppLucideIcon name="locate" /></template>
      {{ t('owner.editor.location.use_current') }}
    </AppButton>
    <p v-if="locationError" class="location-step__notice" role="status">
      {{ locationError }} {{ t('owner.editor.location.manual_available') }}
    </p>
    ...
    <div class="location-step__address">
      <AppInput
        :model-value="draft.address.formatted"
        :label="t('owner.editor.location.address')"
        :placeholder="t('owner.editor.location.address_placeholder')"
        :error="validation.address ? t(validation.address) : undefined"
        required
        @update:model-value="draftStore.setAddress({ formatted: $event })"
      />
      <AppInput
        :model-value="draft.address.city"
        :label="t('owner.editor.location.city')"
        @update:model-value="draftStore.setAddress({ city: $event })"
      />
      <AppInput
        :model-value="draft.address.district"
        :label="t('owner.editor.location.district')"
        @update:model-value="draftStore.setAddress({ district: $event })"
      />
    </div>

    <div class="location-step__coordinates">
      <AppInput
        :model-value="draft.coordinates.latitude?.toString() ?? ''"
        type="number"
        :label="t('owner.editor.location.latitude')"
        :error="validation.latitude ? t(validation.latitude) : undefined"
        :min="-90"
        :max="90"
        :step="0.000001"
        required
        @update:model-value="updateCoordinate('latitude', $event)"
      />
      <AppInput
        :model-value="draft.coordinates.longitude?.toString() ?? ''"
        type="number"
        :label="t('owner.editor.location.longitude')"
        :error="validation.longitude ? t(validation.longitude) : undefined"
        :min="-180"
        :max="180"
        :step="0.000001"
        required
        @update:model-value="updateCoordinate('longitude', $event)"
      />
    </div>
  </div>
</template>
...
.location-step > :deep(.app-button) {
  justify-self: start;
}
.location-step__notice {
...
.location-step__map :deep(.venue-map),
.location-step__map :deep(.venue-map__canvas) {
  min-height: 300px;
}
.location-step__map small {
  color: var(--color-text-secondary);
}
.location-step__address {
  display: grid;
  gap: var(--space-4);
}
.location-step__coordinates {
  display: grid;
