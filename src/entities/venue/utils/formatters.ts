...
export function formatPrice(option: PricingOption | null | undefined, locale = 'en-US'): string | null {
  const amount = formatMoney(option?.price, locale)
  return amount && option ? `${amount} / ${pricingUnitLabels[option.unit]}` : null
}

export function formatDistance(distanceMeters: number | null | undefined, locale = 'en-US'): string | null {
...
export function formatRating(rating: Venue['rating'], locale = 'en-US'): string | null {
  ...
  const score = new Intl.NumberFormat(locale, { minimumFractionDigits: 1, maximumFractionDigits: 1 }).format(rating.average)
  return `${score} (${rating.count.toLocaleString(locale)})`
}

export function formatOperatingStatus(status: VenueOperatingStatus | null | undefined): string | null {
...
export function getVenueImage(venue: Venue, fallback?: string): string | null {
  const categoryFallbacks: Readonly<Record<string, string>> = {
    ...
    football: 'https://images.unsplash.com/photo-1575361204480-aadea25e6e68?auto=format&fit=crop&w=900&q=80',
    gym: 'https://images.unsplash.com/photo-1534438327276-14e5300c3a48?auto=format&fit=crop&w=900&q=80',
  }
  return venue.coverMedia?.url || fallback || categoryFallbacks[venue.category.slug] || categoryFallbacks.gym
}
