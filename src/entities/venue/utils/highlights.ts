...
function fromRule(venue: VenueDetail, rule: CategoryHighlightRule): string | null {
  ...
  if (rule.source === 'amenity')
    return venue.amenities?.find((item) => item.id === rule.key)?.name ?? null
  if (rule.source === 'facility')
    return venue.facilities?.find((item) => item.id === rule.key)?.name ?? null
  if (rule.source === 'activity')
    return venue.activities?.find((item) => item.id === rule.key)?.name ?? null
...
export function getVenueHighlights(
  venue: VenueListItem | VenueDetail,
  limit = 3,
): readonly string[] {
