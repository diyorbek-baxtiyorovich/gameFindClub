...
<template><DetailSection v-if="resources.length" :title="title"><div class="resources"><article v-for="resource in resources" :key="resource.id"><header><h3>{{ resource.name }}</h3><span v-if="resource.quantity">Jami {{ resource.quantity }} ta</span></header><dl v-if="resource.attributes"><template v-for="(attribute, key) in resource.attributes" :key="key"><dt>{{ label(String(key)) }}</dt><dd>{{ key === 'hourlyPrice' ? `${Number(attribute).toLocaleString('uz-UZ')} UZS` : value(attribute) }}</dd></template></dl></article></div></DetailSection></template>
