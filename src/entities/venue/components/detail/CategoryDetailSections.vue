...
const sections = computed(() =>
  (getCategoryByCode(props.venue.category.slug).detailSections ?? [])
    .filter((section) => section.source in CATEGORY_DETAIL_REGISTRY)
    .map((section) => ({
      ...section,
      component: CATEGORY_DETAIL_REGISTRY[section.source as keyof typeof CATEGORY_DETAIL_REGISTRY],
    })),
)
</script>
<template>
  <component
    ...
    :key="section.id"
    :title="section.label.defaultValue"
